...
# А, В, Е, И, Н, О, Р, С, Т – 1 очко; Д, К, Л, М, П, У – 2 очка; Б, Г, Ё, Ь, Я 
# – 3 очка; Й, Ы – 4 очка; Ж, З, Х, Ц, Ч – 5 очков; Ш, Э, Ю – 8 очков; Ф, Щ, Ъ 
# – 10 очков. Напишите программу, которая вычисляет стоимость введенного 
    ...
    for letter in word:
        for key, value in list_English.items():
            if letter.upper() in value:
                summa += key
    print(f'кол-во очков английского слова = {summa}')
else:
    if word[0].lower() in rus:
        ...
        for letter in word:
            ...
            for key, value in list_Russian.items():
                if letter.upper() in value:
                    summa += key
    print(f'кол-во очков русского слова = {summa}')
